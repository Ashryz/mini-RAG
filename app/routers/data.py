...
async def upload_file(project_id: str, file: UploadFile, app_settings: Settings = Depends(get_settings)):
    ...
    is_valid = FileData().validate_file(file=file)
    ...
    project_path = FileData().get_project_path(project_id=project_id)
    new_filename = FileData().generate_unique_filename(filename=file.filename, project_id=project_id)
